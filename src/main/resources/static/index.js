...
Paging.prototype.toPage = function (page){
    ...
    $.ajax({
        ...
        data:({
            pageNo:page,
            pageSize:10,
            jsonstr:JSON.stringify({orderid:orderid,state:state,time1:time1,time2:time2,time3:time3,time4:time4})
        }),
        success:function(res){console.log(res)
            var str = '';
            for (var i = 0; i < res.list.length; i++) {
                str+='<tr>' +
                    '<td nowrap>'+res.list[i].id+'</td>' +
                    '<td nowrap>'+res.list[i].orderid+'</td>' +
                    '<td nowrap>'+res.list[i].price+'</td>' +
                    '<td nowrap>'+res.list[i].creattime+'</td>' +
                    '<td nowrap>'+res.list[i].paytime+'</td>' +
                    '<td nowrap>'+res.list[i].type+'</td>' +
                    '<td nowrap>'+res.list[i].state+'</td>' +
                    '<td nowrap>'+res.list[i].serialnumber+'</td>' +
                    '</tr>';
            }
